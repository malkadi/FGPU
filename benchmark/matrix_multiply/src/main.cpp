...
int main()
{
  ...
  Xil_DCacheEnable();
  // create kernel
  kernel<TYPE> matrix_multiply_kernel(sqrt(MAX_PROBLEM_SIZE), use_vector_types, use_hard_float);
  power_measure power;
  if( sync_power_measurement ) {
    power.set_idle();
  }
  // download binary to FGPU
  matrix_multiply_kernel.download_code();


  matrix_multiply_kernel.print_name();
  ...
  for(size_index = 0; size_index < test_vec_len; size_index++)
  {
    ...
    if(matrix_multiply_kernel.get_problemSize() > MAX_PROBLEM_SIZE){
