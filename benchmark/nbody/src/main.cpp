...
int main()
{
  ...
  nbody_kernel.download_code();


  nbody_kernel.print_name();
  ...
  for(size_index = 0; size_index < test_vec_len; size_index++)
  {
    ...
    if(nbody_kernel.get_problemSize() > MAX_PROBLEM_SIZE){
    ...
    // compute on ARM
    if (!sync_power_measurement ) {
      timer_val_arm[size_index] = nbody_kernel.compute_on_ARM(nruns);
    }
    
    // compute on FGPU
    if (!sync_power_measurement ) 
      timer_val_fgpu[size_index] = nbody_kernel.compute_on_FGPU(nruns, check_results);
    else
      timer_val_fgpu[size_index] = nbody_kernel.compute_on_FGPU(nruns, false);
