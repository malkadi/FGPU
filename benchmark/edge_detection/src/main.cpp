...
int main()
{
  ...
  kernel<TYPE> compass_edge_detection_kernel(maxDim);
  ...
  for(size_index = 0; size_index < test_vec_len; size_index++)
  {
    ...
    compass_edge_detection_kernel.prepare_descriptor(8 << size_index);
    xil_printf("%-8u", compass_edge_detection_kernel.get_problemSize());
    fflush(stdout);

    // break if the requested problem size is set too big by mistake
    if(compass_edge_detection_kernel.get_problemSize() > MAX_PROBLEM_SIZE){
