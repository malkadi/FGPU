...
{
  input = new unsigned[maxDim*maxDim];
...
{
  unsigned i;
  unsigned *input_ptr = (unsigned*) input;
...
void compute_compass_edge_detection(unsigned *input_ptr, unsigned *amplitude_ptr, unsigned *angle_ptr, unsigned Size)
{
...
void compute_sobel(unsigned *in, float* out, unsigned Size)
{
...
unsigned kernel<T>::compute_on_ARM(unsigned int n_runs)
{
  ...
  while(runs < n_runs)
  {
    ...
    if(typeid(T) == typeid(unsigned)) {
      compute_compass_edge_detection(input, (unsigned *)amplitude_arm, (unsigned *)angle_arm, size0);
      // flush the results to the global memory 
      Xil_DCacheFlushRange((unsigned)amplitude_arm, sizeof(unsigned)*size);
      Xil_DCacheFlushRange((unsigned)angle_arm, sizeof(unsigned)*size);
    }
    else {
      compute_sobel(input, (float*) amplitude_arm, size0);
      // flush the results to the global memory 
      Xil_DCacheFlushRange((unsigned)amplitude_arm, sizeof(unsigned)*size);
    }
...
void kernel<T>::check_FGPU_results()
{
  ...
  if (typeid(T) == typeid(unsigned)) {
    for (unsigned i = 1; i < rowLen-1; i++) {
      for (unsigned j = 1; j < rowLen-1; j++) {
    ...
    for (unsigned i = 1; i < rowLen-1; i++) {
      for (unsigned j = 1; j < rowLen-1; j++) {
        if(amplitude_arm[i*rowLen+j] != amplitude_fgpu[i*rowLen+j]) {
          #if PRINT_ERRORS
          if(nErrors < 10)
            xil_printf("res[%d]=%6.2f (must be %6.2f)\n\r", i*rowLen+j, (float)amplitude_fgpu[i*rowLen + j], (float) amplitude_arm[i*rowLen + j]);
          #endif
          nErrors++;
        }
      }
...
{
  if(typeid(T) == typeid(unsigned))
    xil_printf("\n\r" ANSI_COLOR_YELLOW "Kernel is compass edge detection (does not need floats)\n\r" ANSI_COLOR_RESET);
  else if(typeid(T) == typeid(float))
    xil_printf("\n\r" ANSI_COLOR_YELLOW "Kernel is sobel filter (using floats)\n\r" ANSI_COLOR_RESET);
}
template<typename T>
