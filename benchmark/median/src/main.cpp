...
int main()
{
  ...
  kernel<TYPE> median_kernel(maxDim);
  // download binary to FGPU
  median_kernel.download_code();


  median_kernel.print_name();
  ...
  for(size_index = 0; size_index < test_vec_len; size_index++)
  {
    ...
    median_kernel.prepare_descriptor(8 << size_index);
    xil_printf("%-8u", median_kernel.get_problemSize());
    fflush(stdout);

    // break if the requested problem size is set too big by mistake
    if(median_kernel.get_problemSize() > MAX_PROBLEM_SIZE){
      xil_printf("Problem size exceeds limit!\n\r");
      break;
    }

    // compute on ARM
    timer_val_arm[size_index] = median_kernel.compute_on_ARM(nruns);
    
    // compute on FGPU
    timer_val_fgpu[size_index] = median_kernel.compute_on_FGPU(nruns, check_results);
