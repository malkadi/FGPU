...
int main()
{
  ...
  kernel<TYPE> sum_power_kernel(maxProblemSize, use_atomics);
  power_measure power;
  ...
  sum_power_kernel.download_code();


  sum_power_kernel.print_name();
  ...
  for(size_index = 0; size_index < test_vec_len; size_index++)
  {
    ...
    // compute on ARM
    if (!sync_power_measurement ) {
      timer_val_arm[size_index] = sum_power_kernel.compute_on_ARM(nruns);
    }
    
    // compute on FGPU
    if (!sync_power_measurement )
      timer_val_fgpu[size_index] = sum_power_kernel.compute_on_FGPU(nruns, check_results, best_reduce_factor[size_index]);
    else
      timer_val_fgpu[size_index] = sum_power_kernel.compute_on_FGPU(nruns, false, best_reduce_factor[size_index]);
