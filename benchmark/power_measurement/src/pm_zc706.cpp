...
// disables synchronization for power measurement to measure reference power consumption
#define MEASURE_REFERENCE   0
// maximum number of power samples to save
#define MAX_SAMPLES         1024*1024
// maximum number of power samples to save (measuring reference consumption without synchronization)
#define REFERENCE_SAMPLES   10
...
int main()
{
  ...
  raw_uivalues = (struct raw_uivalues_STR*)malloc(MAX_SAMPLES*sizeof(struct raw_uivalues_STR));
  if(raw_uivalues == 0) {
    printf("Not enough memory on heap\n");
    return 1;
  }
  struct raw_uivalues_STR *samples = raw_uivalues;

    /**** Measurements ****/
#if !MEASURE_REFERENCE
  volatile int *msync;
  msync = (volatile int *)MEMADDR_SYNC;
...
#if !MEASURE_REFERENCE
  for (sample_count = 0; (*msync == MEASURE_SYNC2) && (sample_count < MAX_SAMPLES); sample_count++)
#else
  // To measure power consumption before FPGA execution begins
  ...
  unsigned Ds_Number;
  /*Calculate*/
  for(Ds_Number = 0; Ds_Number < sample_count; Ds_Number++)
  {
  ...
  Xil_DCacheFlushRange((unsigned) MEMADDR_RES, 16*sizeof(float));
  *msync = MEASURE_RESP;
