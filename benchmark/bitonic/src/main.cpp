...
int main()
{
  ...
  kernel<TYPE> bitonic_kernel(maxProblemSize);
  power_measure power;
  if( sync_power_measurement ) {
    power.set_idle();
  }
  // download binary to FGPU
  bitonic_kernel.download_code();


  bitonic_kernel.print_name();
  ...
  for(size_index = 0; size_index < test_vec_len; size_index++)
  {
    ...
    // compute on ARM
    if (!sync_power_measurement ) {
      timer_val_arm[size_index] = bitonic_kernel.compute_on_ARM(nruns);
    }
