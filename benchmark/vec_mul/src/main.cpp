...
int main()
{
  ...
  Xil_DCacheEnable();
  // create kernel
  kernel<TYPE> vec_mul_kernel(MAX_PROBLEM_SIZE, use_vector_types, use_hard_float);
  power_measure power;
  if( sync_power_measurement ) {
    power.set_idle();
  }
  // download binary to FGPU
  vec_mul_kernel.download_code();


  vec_mul_kernel.print_name();
  ...
  for(size_index = 0; size_index < test_vec_len; size_index++)
  {
    ...
    vec_mul_kernel.prepare_descriptor(64 << (size_index+13));
    xil_printf("%-8u", vec_mul_kernel.get_problemSize());
    fflush(stdout);

    // break if the requested problem size is set too big by mistake
    if(vec_mul_kernel.get_problemSize() > MAX_PROBLEM_SIZE){
    ...
    // compute on FGPU
    if(sync_power_measurement)
      timer_val_fgpu[size_index] = vec_mul_kernel.compute_on_FGPU(nruns, false);
    else
      timer_val_fgpu[size_index] = vec_mul_kernel.compute_on_FGPU(nruns, check_results);

    // compute on ARM
    if (!sync_power_measurement ) {
      timer_val_arm[size_index] = vec_mul_kernel.compute_on_ARM(nruns);
    }
