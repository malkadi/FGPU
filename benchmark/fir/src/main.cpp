...
int main()
{
  ...
  Xil_DCacheEnable();
  // create kernel
  kernel<TYPE> fir_kernel(MAX_PROBLEM_SIZE, use_vector_types, use_hard_float);
  power_measure power;
  ...
  for(size_index = 0; size_index < test_vec_len; size_index++)
  {
    ...
    fir_kernel.prepare_descriptor(64 << (size_index+0));
    ...
    // compute on FGPU
    if(sync_power_measurement)
      timer_val_fgpu[size_index] = fir_kernel.compute_on_FGPU(nruns, false);
    else
